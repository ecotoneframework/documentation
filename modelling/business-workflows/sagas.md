...
**The saga survives anything that doesn't destroy your database.** Every time a saga handles an event, its updated state is committed to the database in the same transaction as the events it publishes. If the process dies mid-handler, the channel redelivers the message (delivery is at-least-once); the saga is reloaded from storage and the handler runs again — and because a re-dispatched command is tolerated by [deduplication](../recovering-tracing-and-monitoring/resiliency/idempotent-consumer-deduplication.md), the **Timing difference** below (when a saga sends a command via the `CommandBus` before its state is saved) doesn't lose or double-apply work. Deploys, restarts, OOM kills — none of them lose work, because the work isn't in memory.
